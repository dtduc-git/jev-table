...
from jev_table.output import answer_columns, build_rows, output_columns, read_csv, write_corrections
...
def test_corrections_hold_only_review_rows_without_errors(tmp_path: Path) -> None:
    ...
    count = write_corrections(corrections, ["message"], out, spec)
    assert count == 1
...
def test_corrections_not_written_when_nothing_flagged(tmp_path: Path) -> None:
    ...
    assert out[0]["review"] == ""
    corrections = tmp_path / "corrections.csv"
    assert write_corrections(corrections, ["message"], out, spec) == 0
